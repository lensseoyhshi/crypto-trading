...
class BinanceExchange(BaseExchange):
    ...
    def _initialize_exchange(self):
        ...
        try:
            ...
            logger.info(f"Binance exchange initialized (sandbox: {self.sandbox})")
    ...
    async def create_order(self, symbol: str, side: OrderSide, order_type: OrderType,
        ...
        try:
            ...
            logger.info(f"Binance order created: {order_response.exchange_order_id}")
            
            return order_response
            
        except Exception as e:
            logger.error(f"Failed to create Binance order: {e}")
            raise
    ...
    async def get_orders(self, symbol: Optional[str] = None,
        ...
        try:
            ...
            logger.error(f"Failed to get Binance orders: {e}")
            raise
    ...
    async def get_klines(self, symbol: str, interval: str, limit: int = 100,
        ...
        try:
            ...
            # Fetch OHLCV data
            ohlcv = await self.exchange.fetch_ohlcv(symbol, binance_interval, limit=limit, **params)
            ...
            for data in ohlcv:
                kline = Kline(
                    ...
                    close_time=datetime.fromtimestamp(data[0] / 1000),  # Simplified
            ...
            logger.error(f"Failed to get Binance klines for {symbol}: {e}")
            raise
