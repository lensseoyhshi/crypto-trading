...
class OKXExchange(BaseExchange):
    ...
    def _initialize_exchange(self):
        ...
        try:
            ...
            logger.info(f"OKX exchange initialized (sandbox: {self.sandbox})")
    ...
    async def create_order(self, symbol: str, side: OrderSide, order_type: OrderType,
        ...
        try:
            ...
            logger.info(f"OKX order created: {order_response.exchange_order_id}")
    ...
    async def get_order(self, order_id: str, symbol: str) -> OrderResponse:
        ...
        try:
            ...
            logger.error(f"Failed to get OKX order {order_id}: {e}")
            raise
    ...
    async def close_position(self, symbol: str, side: PositionSide,
        ...
        try:
            ...
            logger.error(f"Failed to close OKX position: {e}")
            raise
    ...
    async def get_ticker(self, symbol: str) -> MarketData:
        ...
        try:
            ...
            logger.error(f"Failed to get OKX ticker for {symbol}: {e}")
            raise
    ...
    async def get_klines(self, symbol: str, interval: str, limit: int = 100,
        ...
        try:
            ...
            # Fetch OHLCV data
            ohlcv = await self.exchange.fetch_ohlcv(symbol, okx_interval, limit=limit, **params)
            ...
            logger.error(f"Failed to get OKX klines for {symbol}: {e}")
            raise
